...
function ADDcard(d) {
  ...
  for (let i = 0; i < d.length; i++) {
    console.log(d[i])
    if(d[i].flag[0]==='i'){
      var node = `<div class="card" style="width: 18rem;"> <img src="../${d[i].flag}" class="card-img-top" alt="..."> <div class="card-body"> <p class="fs-1">${d[i].name}</p> <p class="fs-3">Continent :${d[i].continent}</p> <br> <p class="fs-3">Rank :${d[i].rank}</p> <br>  </div> </div>`;
      var cardnode = document.createElement("DIV");
      cardnode.innerHTML = node;
      document.getElementById("country").appendChild(cardnode);
    }
    if(d[i].flag[0] !=='i'){
      var node = `<div class="card" style="width: 18rem;"> <img src="../images/${d[i].flag}" class="card-img-top" alt="..."> <div class="card-body"> <p class="fs-1">${d[i].name}</p> <p class="fs-3">Continent :${d[i].continent}</p> <br> <p class="fs-3">Rank :${d[i].rank}</p> <br>  </div> </div>`;
      var cardnode = document.createElement("DIV");
      cardnode.innerHTML = node;
      document.getElementById("country").appendChild(cardnode);
    }
...
function sendcountry() {
  ...
  let flag = document.getElementById('flag').files[0].name;
   axios
...
var loadFile = function(event) {
  ...
  let continent = document.getElementById("continent").value.toString();
  let rank = document.getElementById("rank").value;
   document.getElementById('nameofimage').value = event.target.files[0].name
